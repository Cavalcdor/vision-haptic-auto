...
智能汽车交互界面全面触屏化使驾驶员因缺乏物理触觉线索被迫转移视线，导致驾驶分心与安全隐患。本文提出一种基于视触觉感知的车载力觉反馈系统：通过屏下摄像头捕捉形变图像，结合标记点检测与光流追踪，提取力度、面积、速度和位置四维参数；基于映射模型与场景策略驱动压电阵列产生差异化反馈。系统构建"感知-决策-执行"三层闭环架构，预期实现力度分辨率≤0.05N、延迟<10ms、可区分效果≥8种。
